...
**知识被编译一次，然后持续维护**，而不是每次重新推导。这是本系统和 RAG/聊天记录的根本区别：RAG 每次都从原始文件推导，没有积累；llm-wiki 把推导结果保存下来，后续在已有结构上增量更新。

**设计取舍（执行时需要理解的背景）**：

- **为什么是 `[[wikilink]]` 而不是 `[text](path)`**：wikilink 是意图链接（"这个概念指向另一个概念"），path 链接是文件位置依赖。知识库里的页面会移动、合并、拆分；意图链接比路径更稳定，也更容易被 lint 脚本检测断链。选错格式的成本：页面迁移后一批链接静默失效，lint 追不到。

- **为什么是本地 markdown 而不是数据库**：markdown 文件本身就是人类可读的知识表示，任何编辑器都能独立打开，知识所有权属于你。数据库格式让知识库变成工具依赖，工具消失则知识锁死。

- **页面粒度的取舍**：太细（每个事实一页）→ 导航噪音大，连接密度低，索引膨胀；太粗（每个话题一页）→ 同一概念散落多处，复用难。经验规则：一个有独立意义的**概念/实体/事件**值得一页；一条观点、一条数据不值得单独一页——放入相关页的正文或 frontmatter。

- **何时不该 ingest**：有强时效性的通知（活动时间、限时优惠）、高度上下文依赖的个人备忘、纯闲聊内容。这类内容进知识库后价值衰减极快，是噪声而非知识。
...
**为什么需要这步**：llm-wiki 无法自动识别用户素材中的敏感信息——正则在非结构化文本（聊天记录、笔记）里误报率很高，同时也会漏掉真正的敏感词。把判断权交给用户，比让脚本自动决定更可靠，也避免用户遇到看不懂的脚本报错。代价是增加一次交互，但对用户的知识库安全来说是值得的。

**触发条件**：当前对话里第一次进入 ingest 时执行。如果用户在本次对话中已明确说过"素材里没有敏感信息，直接开始"，或者在 `batch-ingest` 顶层已确认过，跳过此步。

在开始提取或分析任何内容之前，先对用户说下面这句话，然后等待确认：
...
5. **Step 1：结构化分析**：
   ...
   **置信度字段的目的**：让消费这份知识的读者（包括未来的你）知道每条信息的证据强度。错误分类的成本是：读者无法区分"字面事实"和"推断猜测"，知识库失去可信度。四个级别：
   - `EXTRACTED`：信息直接出现在原文里，字面可以找到
   - `INFERRED`：信息是从多处原文推断出来的，原文没有直接说
   - `AMBIGUOUS`：原文说法不清楚，或者有歧义
   - `UNVERIFIED`：信息来自背景知识，原文没有证据（风险最高，需谨慎）

   **为什么在进入 Step 2 前验证**：Step 1 的 JSON 是 Step 2 全部页面生成的依据。如果格式异常（缺少 `confidence` 或 `entities` 字段）在 Step 2 才发现，已经写入了一批 wiki 页面，回退成本很高。提前验证让失败发生在写入前。
...
2. **Step 0：调用脚本做机械检查**（先于语义分析执行，原因：脚本负责精确枚举，AI 负责语义判断；如果先做语义分析再发现孤立页，修复建议可能已经基于不完整的图结构给出了）：
